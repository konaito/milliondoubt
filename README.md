...
ミリオンダウトをブラウザで遊べる、GitHub Pages対応の静的アプリです。公開情報だけを使う戦略レイヤーで合法手と終盤を整え、その上で自己対戦NNをタイブレークに使います。
...
学習方法と観測情報の設計は [NN_TRAINING.md](NN_TRAINING.md) にまとめています。学習側は戦略教師を混ぜた自己対戦で、ブラウザ側は合法性・最小限の返し・手札切れ・8切りを戦略レイヤーで先に評価します。NNは評価が近い候補の選択に使うため、短い学習やモデルの癖で明らかな悪手を選びにくくしています。PyTorchチェックポイントをブラウザ用に変換するには、学習環境で次を実行します。
